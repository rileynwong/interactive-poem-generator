...
;(function($) {
	$.fn.scriptcam=function(options) {
		...
		return this.each(function() {
			...
			if (fileExists(decodeURIComponent(data.path)+'static/scriptcam.swf')) {
				swfobject.embedSWF(decodeURIComponent(data.path)+'static/scriptcam.swf', opts.id, newWidth, newHeight, '11.6', false, opts, params);
			}
			else {
				alert(decodeURIComponent(data.path)+'static/scriptcam.swf not found, please check the path parameter');
			}
		});
	};
